...
router.post("/", trackingRateLimit, async (req, res, next) => {
    try {
        const { website, time, pageTitle } = req.body;
        ...
        if (catDoc && catDoc.source === "user") {
            ...
            // Use AI classifier
            const aiResult = aiClassifier.classify(website, pageTitle || "");
...
router.get("/stats", async (req, res, next) => {
    try {
        ...
        res.json({ score, totalTime, productiveTime, streak, peakHour });
    } catch (err) {
        next(err);
    }
});
...
router.get("/cognitive-load", async (req, res, next) => {
    try {
        ...
        const result = data.map(d => {
            ...
            // Basic heuristic: 1 session per minute is baseline (5). 
            // 2 sessions/min is high (8+).
            const sessionsPerMin = d.sessionCount / (d.totalTime / 60 || 1);
            let loadScore = Math.min(10, Math.max(1, sessionsPerMin * 5));

            // If very low total time in that hour, soften the score
            if (d.totalTime < 300) loadScore = loadScore * 0.5;

            return { time, loadScore: parseFloat(loadScore.toFixed(1)) };
        });

        // Ensure at least some data points for the UI if there's no recent activity
        if (result.length === 0) {
            for (let i = 5; i >= 0; i--) {
                result.push({
